...
For my second modification, I attached wheels to my robot.  But not just any type of wheel, tank treads in order to make my robot look like the actual Walle robot we see in pixar movies.  With the addition of these tank treads, I had to put the frame together using various types of screws.  Additionally, I also had to use standoffs in order to elevate the height of my Walle robot so it didn't come into contact with the wheels.  After the securing the Walle robot on top of the tank tread frame, I ran into another problem.  The motor driver had nowhere to go and was just awkwarding hanging around.  As a result, I drilled holes into the bottom of my Walle robot so I could the motor driver inside the robot and also for the standoffs to be placed.  One more challenge I faced was that the arduino was powered by my computer, so if it actually ran, the length of the wire would not be enough.  I fixed this problem by attaching a 9V battery to the arduino so it wouldn't have to rely on my computer power anymore.  Then, I also coded the bluetooth to include the turning of the wheels.  Finally, my finished project was a walle robot that could display different emotions using an ultrasonic sensor and bluetooth, but could also move using the bluetooth.
...
| USB Power Wall Adapter | Getting power to arduino | $6.99 | <a href="https://www.amazon.com/Charger-Adapter-Ailkin-Replacement-Samsung/dp/B0734SN6VR?source=ps-sl-shoppingads-lpcontext&ref_=fplfs&psc=1&smid=A7KYCJDAP20NG"> Link </a> |
| Cables(40 male to female, 40 female to male) | Connecting everything | $ Varies | <a href="https://store-usa.arduino.cc/collections/cables-wires?selectedStore=us"> Link </a> |
| HM-10 Bluetooth Module | Bluetooth connections | $10.99 | <a href="https://www.amazon.com/DSD-TECH-Bluetooth-iBeacon-Arduino/dp/B06WGZB2N4"> Link </a> |
